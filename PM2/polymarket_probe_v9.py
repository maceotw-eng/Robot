...
def _batch(params):
    try:
        ...
        if getattr(exc, "code", None) == 422 and int(params.get("offset", 0)) > 0:
            # Depth wall only makes sense past offset 0. Carry the API's own
            # words AND the exact params - a wall report you have to re-derive
            # by hand is a wall report that failed (lesson: v6's first 422
            # arrived as a bare "deep-offset wall" with the true cause,
            # "invalid time range", thrown away by this very function).
            raise DepthWall("HTTP 422 at offset=%s | api said: %s | params: %s"
                            % (params.get("offset"),
...
def _page_span(params_base, ceiling, seen_ids, markets, tag):
    """
    Page one query (fixed filters + order) up to the offset ceiling.
    Returns (rows_read, ceiling_hit, min_id, max_id, new_count).
    """
    offset = 0
    rows_read = 0
    new_count = 0
    ...
    while offset <= ceiling:
        ...
        if not isinstance(batch, list) or not batch:
            return rows_read, False, min_id, max_id, new_count
        rows_read += len(batch)
        for market in batch:
            mid = market.get("id")
            try:
            ...
            if mid_n is not None:
                if min_id is None or mid_n < min_id:
                    min_id = mid_n
                if max_id is None or mid_n > max_id:
                    max_id = mid_n
            if mid is not None and mid in seen_ids:
                continue
            if mid is not None:
                seen_ids.add(mid)
            if len(CENSUS_POOL) < CENSUS_POOL_MAX:
                CENSUS_POOL.append(market)
            row = slim_market(market)
            markets.append(row)
            if _page_span.stream is not None:
                _page_span.stream.write(json.dumps(row) + "\n")
            new_count += 1
        if _page_span.pages % 50 == 0:
            sys.stderr.write("\n[hb %s] pages=%d markets=%d\n"
                             % (datetime.now(timezone.utc).strftime("%H:%M:%SZ"),
                                _page_span.pages, len(markets)))
            if _page_span.stream is not None:
                _page_span.stream.flush()
        offset += len(batch)
        sys.stderr.write("\r  %s offset %-5d | +%d new | %d total   "
                         % (tag, offset, new_count, len(markets)))
        sys.stderr.flush()
        if _page_span.pages >= _page_span.budget:
            return rows_read, True, min_id, max_id, new_count
    return rows_read, True, min_id, max_id, new_count
...
def _as_ts(day_or_ts, end=False):
    if "T" in day_or_ts:
        return day_or_ts
    return day_or_ts + ("T23:59:59Z" if end else "T00:00:00Z")


def _server_floor_verified(date_min, floor):
    """
    Gamma SILENTLY IGNORES unknown parameters (measured: id_min returned the
    smallest ids as if the filter were absent). So before relying on
    volume_num_min we verify it twice, in both directions:
      1. a page WITH the filter must contain only rows >= floor
      2. a control page WITHOUT it, same window, must contain at least one
         row < floor (proving the filter actually removed something rather
         than the window happening to hold only large markets)
    Only if both hold is the server-side floor trusted.
    """
    try:
        probe = {"closed": "true", "limit": 100, "order": "id",
                 "ascending": "true", "volume_num_min": floor,
                 "end_date_min": _as_ts(date_min or "2024-01-01"),
                 "end_date_max": _as_ts(date_min or "2024-01-01", end=True)
                 .replace("T23:59:59Z", "T23:59:59Z")}
        # bounded one-day window: the v8 verifier sent an unbounded query and
        # 422'd, reporting OFF while the data came back looking filtered -
        # the exact "instrument says one thing, data says another" state this
        # project exists to kill.
        probe["end_date_max"] = _as_ts(
            (date_min or "2024-01-01"), end=True)
        ctrl = dict(probe)
        del ctrl["volume_num_min"]
        with_f = _batch(probe)
        without = _batch(ctrl)
        if not isinstance(with_f, list) or not with_f:
            return False, "filtered page empty"
        vols_f = [volume_of(m)[0] for m in with_f]
        vols_c = [volume_of(m)[0] for m in (without or [])]
        if min(vols_f) < floor:
            return False, "filter not honoured (sub-floor rows came back)"
        if vols_c and min(vols_c) >= floor:
            return False, "control inconclusive (window has no sub-floor rows)"
        return True, "verified: filtered page >= %s, control has sub-floor rows" % floor
    except Exception as exc:
        return False, "verification errored: %s" % exc


def harvest_markets(max_pages, page_size=500, date_min=None, date_max=None,
                    order="id", ascending=True, server_floor=None,
                    state_dir=None):
    """
    Harvest closed markets across the charter window.

    v6: DATE-SPAN BISECTION with ID-SORTED PAGING. Faults that forced each
    redesign, every one only visible at a different real-world scale:

    1. v1 short-page stop        - Gamma caps ~100 rows/page however large a
                                   limit you ask; breaking on a short page
                                   truncated the harvest to page one.
    2. v2 deep-offset wall       - offset<=2000 ok, >=2100 -> HTTP 422.
    3. v3 single dense day       - a date cursor cannot advance past a day
                                   holding more rows than the ceiling.
    4. v5 CONTINUOUS dense days  - from 2025-11 onward EVERY day exceeds the
                                   ceiling (hourly/short markets), so "detour
                                   around the odd busy day" is no design at
                                   all. Meanwhile id_min proved to be SILENTLY
                                   IGNORED by Gamma (unknown params do not
                                   error), so a pure id cursor is unavailable.

    Design: keep a stack of [lo, hi] day spans. Page each span with order=id
    ascending up to the ceiling. Ceiling hit -> bisect the span and rescan the
    halves (dedupe by id makes the overlap harmless). A single day that still
    overflows is scanned from BOTH ends (id ascending then descending); if the
    two passes' id ranges touch, coverage of that day is proven complete. Only
    a single day deeper than ~2x the ceiling remains unreachable, and that is
    reported as DepthWall - loudly, never as a silently partial harvest.
    """
    markets, seen_ids = [], set()
    stack = [(_as_ts(date_min or "2020-01-01"),
              _as_ts(date_max or "2030-12-31", end=True))]
    _page_span.pages = 0
    _page_span.budget = max_pages
    _page_span.stream = None
    CENSUS_POOL.clear()

    # ---- resume state: a $5 VPS can die mid-flight (v7 vanished without a
    # traceback at 297K rows); completed leaf spans and streamed rows survive
    # on disk, so a rerun re-reads none of them. ----
    journal_path = stream_path = None
    done_ivs = []
    journal_fh = None
    if state_dir:
        os.makedirs(state_dir, exist_ok=True)
        stream_path = os.path.join(state_dir, "harvest_stream.ndjson")
        journal_path = os.path.join(state_dir, "harvest_journal.txt")
        ...
        if os.path.exists(journal_path):
            with open(journal_path) as fh:
                for line in fh:
                    ...
                    if len(parts) >= 2:
                        try:
                            done_ivs.append([int(parts[0]), int(parts[1])])
                        except ValueError:
                            pass
            done_ivs = _iv_merge(done_ivs)
        if markets or done_ivs:
            sys.stderr.write("  RESUME: %d rows and %d completed intervals "
                             "loaded from %s\n"
                             % (len(markets), len(done_ivs), state_dir))
        _page_span.stream = open(stream_path, "a")
        journal_fh = open(journal_path, "a")
    bisections = 0
    dense_days = []
    truncated = False

    base = {"closed": "true", "limit": page_size, "order": "id"}
    floor_used, floor_note = False, "not requested"
    if server_floor:
        floor_used, floor_note = _server_floor_verified(date_min, server_floor)
        if floor_used:
            base["volume_num_min"] = server_floor
        sys.stderr.write("  server volume floor: %s (%s)\n"
                         % ("ON" if floor_used else "OFF", floor_note))

    while stack:
        if _page_span.pages >= max_pages:
            truncated = True
            break
        lo, hi = stack.pop()
        lo_e0, hi_e0 = _ts_epoch(lo), _ts_epoch(hi)
        if done_ivs and _iv_covered(done_ivs, lo_e0, hi_e0):
            continue                          # finished in a previous run
        span_base = dict(base, ascending="true",
                         end_date_min=lo, end_date_max=hi)

        # PROBE PAGE first: one request at offset=ceiling tells us whether
        # this span overflows, without re-reading up to 20 pages of rows the
        # parent span already harvested. Bisection descent thus costs one
        # page per tree node instead of twenty (a draft of this version
        # burned its whole page budget re-confirming density level by level
        # down a 25-deep timestamp tree).
        probe_rows = _batch(dict(span_base, offset=OFFSET_CEILING))
        _page_span.pages += 1
        overflow = isinstance(probe_rows, list) and len(probe_rows) > 0
        if isinstance(probe_rows, list):
            for market in probe_rows:         # free rows - keep them
                mid_ = market.get("id")
                if mid_ is not None and mid_ not in seen_ids:
                    seen_ids.add(mid_)
                    if len(CENSUS_POOL) < CENSUS_POOL_MAX:
                        CENSUS_POOL.append(market)
                    row_ = slim_market(market)
                    markets.append(row_)
                    if _page_span.stream is not None:
                        _page_span.stream.write(json.dumps(row_) + "\n")
        lo_e, hi_e = _ts_epoch(lo), _ts_epoch(hi)
        if overflow and hi_e - lo_e >= 3:  # >=3 keeps both children lo<hi
            # Too dense for one pass: bisect the TIME span (down to seconds -
            # measured 2026-08-26: end_date_min/max accept full timestamps,
            # while a zero-length day range is refused as "invalid time
            # range", so lo < hi is kept invariant). Rows already harvested
            # are KEPT; the halves re-read and id-dedupe skips them.
            mid_e = lo_e + (hi_e - lo_e) // 2
            stack.append((_ts_iso(mid_e + 1), hi))
            stack.append((lo, _ts_iso(mid_e)))
            bisections += 1
            continue

        rows, hit, min_a, max_a, _new = _page_span(
            span_base, OFFSET_CEILING, seen_ids, markets,
            "span %s..%s asc" % (lo, hi))
        if _page_span.pages >= max_pages:
            truncated = True
            break
        if not hit:
            if journal_fh:
                journal_fh.write("%d|%d\n" % (lo_e0, hi_e0))
                journal_fh.flush()
            continue                          # span fully read
        # ---- a 1-2 second span still over the ceiling: dual scan ---------
        dense_days.append(lo)
        desc_base = dict(base, ascending="false",
                         end_date_min=lo, end_date_max=hi)
        rows_d, hit_d, min_d, max_d, _new_d = _page_span(
            desc_base, OFFSET_CEILING, seen_ids, markets,
            "day  %s desc" % lo)
        covered = (min_d is not None and max_a is not None
                   and min_d <= max_a + 1)
        if covered and journal_fh:
            journal_fh.write("%d|%d\n" % (lo_e0, hi_e0))
            journal_fh.flush()
        if not covered:
            raise DepthWall(
                "span %s..%s exceeds ~2x the offset ceiling: ascending pass "
                "topped out at id %s, descending pass bottomed at id %s, ids "
                "between are unreachable. A further slicer is needed for "
                "this span." % (lo, hi, max_a, min_d))

    sys.stderr.write("\n")
    if truncated:
        sys.stderr.write("  NOTE: stopped at max_pages=%d - coverage is PARTIAL, "
                         "raise --max-pages\n" % max_pages)
    sys.stderr.write("  harvested %d unique markets | %d bisections | "
                     "%d dense days dual-scanned\n"
                     % (len(markets), bisections, len(dense_days)))
    if _page_span.stream is not None:
        _page_span.stream.close()
        _page_span.stream = None
    if journal_fh:
        journal_fh.close()
    subfloor = sum(1 for m in markets
                   if volume_of(m)[0] < (server_floor or 0))
    stream_lines = None
    if stream_path and os.path.exists(stream_path):
        with open(stream_path) as fh:
            stream_lines = sum(1 for _ in fh)
    integrity_ok = (stream_lines is None or stream_lines == len(markets))
    if not integrity_ok:
        sys.stderr.write("  INTEGRITY FAIL: stream=%s rows vs memory=%d - "
                         "master list on disk is NOT the harvest\n"
                         % (stream_lines, len(markets)))
    HARVEST_META.clear()
    HARVEST_META.update({"pages_used": _page_span.pages, "max_pages": max_pages,
                         "bisections": bisections,
                         "dense_days_dual_scanned": len(dense_days),  # 1-2s spans
                         "server_volume_floor_used": floor_used,
                         "server_volume_floor_note": floor_note,
                         "truncated": truncated,
                         "markets_harvested": len(markets),
                         "resumed_from_state": bool(done_ivs),
                         "base_params_sent": {k: v for k, v in base.items()
                                              if k != "limit"},
                         "subfloor_rows_seen": subfloor,
                         "stream_lines": stream_lines,
                         "stream_integrity_ok": integrity_ok})
    return markets


SLIM_KEYS = ("id", "endDate", "startDate", "closed", "volumeNum", "volume",
             "outcomePrices", "clobTokenIds", "umaResolutionStatuses",
             "category", "tags", "liquidityNum", "question")

# First N full (un-slimmed) rows kept aside so the G0 field census still
# measures the REAL schema rather than our own slimming choices.
CENSUS_POOL = []
CENSUS_POOL_MAX = 300
...
def slim_market(m):
    """
    Keep only charter-relevant fields. Measured on the v7 run that died at
    297,175 rows: full Gamma rows (thumbnails, descriptions, slugs) average
    3-5 KB, so a whole-universe harvest carries >1 GB in RAM on a 1 GB box.
    Slim rows are ~0.3 KB: the same universe fits in ~100 MB.
    """
    s = {k: m[k] for k in SLIM_KEYS if k in m}
    q = s.get("question")
    if isinstance(q, str) and len(q) > 120:
        s["question"] = q[:120]
    return s
...
def probe_checkpoints(sample, fidelity_plan=None):
    """
    Charter 3: price at T days before resolution, T in {1,7,30}.

    v4 BUG, fixed here: v4 fetched ONE series per market with window_days=35 at
    whatever single fidelity G3 chose. At fidelity=1 a 35-day span is rejected
    by the span wall, the interval=max fallback returns empty at minute
    granularity, and G4 reported 0 markets with history while G3 had just
    proven 74% of the same sample serves minute data. Two gates contradicting
    each other on the same sample is the instrument confessing.

    Each checkpoint now uses its own (fidelity, window) sized to what it
    measures: T=1 wants minute data in the last day; T=30 only needs daily.
    """
    plan = fidelity_plan or {1: (1, 2), 7: (60, 14), 30: (1440, 35)}
    present = {t: 0 for t in CHECKPOINTS}
    absent = {t: 0 for t in CHECKPOINTS}
    gaps = {t: [] for t in CHECKPOINTS}
    no_history = {t: 0 for t in CHECKPOINTS}
    fetch_error = {t: 0 for t in CHECKPOINTS}
    usable = 0
    ...
    for idx, m in enumerate(sample):
        ...
        for t in CHECKPOINTS:
            fid, days = plan[t]
            hist, err, _mode = price_history(tokens[0], fid, end_ms,
                                             window_days=days)
            if err or not hist:
                absent[t] += 1
                if err:
                ...
                continue
            any_hist = True
            stamps = [_norm_ts_ms(p["t"]) for p in hist
    ...
    out = {"fidelity_plan": {("T=%d" % t): {"fidelity": plan[t][0],
                                            "window_days": plan[t][1]}
                             for t in CHECKPOINTS},
           "markets_with_history": usable,
           "absent_breakdown": {("T=%d" % t): {"no_history": no_history[t],
                                               "fetch_error": fetch_error[t]}
                                for t in CHECKPOINTS},
           "checkpoints": {}}
    for t in CHECKPOINTS:
...
def main():
    ...
    # Filter audit: one bounded query WITHOUT the floor param. If sub-floor
    # rows exist out there but zero appear in our harvest, the effective
    # filter state is ON no matter what any verification step claimed.
    audit = {"attempted": True}
    try:
        ctrl = get(GAMMA + "/markets",
                   {"closed": "true", "limit": 100, "order": "id",
                    "ascending": "true",
                    "end_date_min": "2024-06-01T00:00:00Z",
                    "end_date_max": "2024-06-07T23:59:59Z"})
        vols = sorted(volume_of(m)[0] for m in (ctrl or []))
        audit.update({"rows": len(vols),
                      "subfloor_rows_no_param": sum(1 for v in vols
                                                    if v < VOLUME_FLOOR),
                      "min_volume_no_param": vols[0] if vols else None})
        audit["effective_filter_state"] = (
            "ON" if (audit["subfloor_rows_no_param"] > 0
                     and HARVEST_META.get("subfloor_rows_seen", 0) == 0
                     and HARVEST_META.get("base_params_sent", {})
                         .get("volume_num_min"))
            else "OFF_or_indeterminate")
    except Exception as exc:
        audit.update({"error": str(exc)})
    result["filter_audit"] = audit
    ...
    # on FULL rows kept aside before slimming, so the census reports the
    # real API schema rather than our own field selection.
    census = field_census(CENSUS_POOL if CENSUS_POOL else markets)
    ...
    # G4 uses a per-checkpoint (fidelity, window) plan — see probe_checkpoints.
    print("\n[G4] checkpoint absence, per-checkpoint fidelity plan ...")
    checks = probe_checkpoints(sample)
    ...
    finest = gran["finest_fidelity_that_returns_data"]
    print("finest fidelity distribution (min): %s" % json.dumps(finest))
