...
def harvest_markets(max_pages, page_size=500, date_min=None, date_max=None,
    ...
    stack = [(date_min or "2020-01-01", date_max or "2030-12-31")]
    _page_span.pages = 0
    ...
    while stack:
        ...
        if _page_span.pages >= max_pages:
            ...
            break
        if lo != hi:
            # Too dense for one pass: bisect and rescan the halves. Rows
            # already harvested are KEPT - the halves re-read the span and
            # id-dedupe skips them, so nothing is lost and little is wasted.
            mid = _day_mid(lo, hi)
            if mid == lo:
                mid_next = _day_shift(lo, 1)
                stack.append((mid_next, hi))
                stack.append((lo, lo))
            else:
                stack.append((_day_shift(mid, 1), hi))
                stack.append((lo, mid))
            bisections += 1
            continue
        # ---- single dense day: scan from the top end as well -------------
        dense_days.append(lo)
        desc_base = dict(base, ascending="false",
                         end_date_min=lo, end_date_max=lo)
        ...
        if not covered:
            raise DepthWall(
                "endDate %s exceeds ~2x the offset ceiling: ascending pass "
                "topped out at id %s, descending pass bottomed at id %s, ids "
                "between are unreachable. A further slicer (e.g. start_date "
                "sub-ranges) is needed for this day." % (lo, max_a, min_d))
    ...
    HARVEST_META.update({"pages_used": _page_span.pages, "max_pages": max_pages,
                         ...
                         "dense_days_dual_scanned": len(dense_days),
